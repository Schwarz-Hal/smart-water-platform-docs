...
算法 `patchtst@0.1.0`、算子 `patchtst@1.0.0` 用于单通道规则时序预测。可作为较复杂预测模型候选，与轻量基线在相同数据与预算下比较。算法名称不代表已在当前水务数据上优于其他模型。
...
使用 Transformers 官方 PatchTSTForPrediction，以 8 点分片、4 点步长、32 维表示、两层和四个注意力头预测；当前只有一个输入通道，不建立管网图。
...
[Transformers PatchTST 官方文档](https://huggingface.co/docs/transformers/model_doc/patchtst)。使用 Apache-2.0 的 Transformers 实现。 平台依据 `patchtst/versions/0.1.0` 模型、训练/推理实现和对应 manifest。

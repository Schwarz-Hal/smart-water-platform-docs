...
- **任务筛选**：通用任务中心按任务类型和状态筛选；数据中心的治理任务另提供创建人和日期等更多筛选。
- **任务信息**：通用列表显示创建时间、更新时间和尝试次数；Worker 信息在任务详情查看。治理列表另外显示执行耗时。
...
- **任务取消与重运行**：
  ...
  - 对失败任务，使用【重新运行】创建新运行实例；治理工作台还提供【按原配置重跑】与【修改配置重跑】，不会覆盖原结果。
